...
async function fixture() {
  ...
  const app = await service.registerService({ name: 'TTL app', port: 5173, protocol: 'http', source: 'manual' })
  const input = { serviceId: app.id, deviceId: device.id }
  return { service, store, clock, input, advance: (ms: number) => { now += ms } }
}
...
test('AI open defaults to 10080 minutes and rejects an authorization longer than a week', async () => {
  const f = await fixture()
  const tool = createCompanionTools(f.service).find(t => t.name === 'task_forward_open')!
...
test('authenticated HTTP open shares the week default and enforces its upper boundary', async t => {
  const f = await fixture()
  const route = createCompanionHttpRoute(f.service, [], undefined, undefined, undefined, { requestRejection: req => req.headers.cookie === 'test=yes' ? undefined : 401 })
  const server = createServer((req, res) => { void route.handler(req, res) })
  server.listen(0, '127.0.0.1'); await once(server, 'listening')
  t.after(async () => { server.closeAllConnections(); await new Promise<void>(resolve => server.close(() => resolve())) })
  const url = 'http://127.0.0.1:' + (server.address() as { port: number }).port + '/api/companion/services/' + f.input.serviceId + '/leases'
