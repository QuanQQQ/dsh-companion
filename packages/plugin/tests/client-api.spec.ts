...
test('browser week option sends exactly seven days in milliseconds', async t => {
  ...
  await openLease('service', 'device', 10080)
})
...
test('browser approval and denial send the JSON media type required by Host', async t => {
  ...
  await unregisterService('service/a')
  await leaseAction('lease/a', 'close')
  assert.deepEqual(paths,['/api/companion/enrollments/own-id/approve','/api/companion/enrollments/own-id/deny','/api/companion/services/service%2Fa/unregister','/api/companion/leases/lease%2Fa/close'])
})
