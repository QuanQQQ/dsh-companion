...
- [ ] 同一命令重跑时验证原配对、停止旧进程、启动新版本；同版本也重新启动，不改变 Lease、TTL 或安全阻断。
...
- [ ] 网络抖动/WSS 丢失关闭自有 SSH，重连先快照；CLI 0.1.11 在连续超过五次控制通道、Forward Instance 或控制器本地瞬态失败后仍退避重试，Host 为 `recovering` 快照重新下发 fenced Open，无需人工重新检查；SSH 清理无法确认时必须停止自动连接。
...
- [ ] Task/Session 结束或归档不影响有效 Lease；Device 撤销、服务注销、TTL 到期和旧 Connection Session 重放无法继续或复活转发。
