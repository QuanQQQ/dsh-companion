...
CLI 修复必须安装到 Device；单独升级 Host 不会替换已运行的 CLI。Host 分发新版后，在 Device 重跑该 Host 页面生成的统一命令，并用已安装 CLI 的 `--version` 确认版本为 0.1.11 或更新。status 中的 `nextReconnectAt`、`lastDisconnectReason` 等是持久诊断，不等同实时在线；认证、证书、协议、凭证、状态写入或清理失败等安全阻断不会因后台重启自动清除。
